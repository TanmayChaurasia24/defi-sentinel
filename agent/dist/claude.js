 ...
 */
async function executeMcpTool(toolName, toolInput, context) {
    switch (toolName) {
        case 'get_wallet_balance': {
            // Try real MCP server first
            const walletAddr = toolInput.wallet_address || context.walletAddress;
            const mcpResult = await (0, mcp_1.callMcpTool)('get_account_balance', {
        ...
        case 'get_cspr_price': {
            // Try real MCP server first
            const mcpResult = await (0, mcp_1.callMcpTool)('get_network_status', {});
            if (mcpResult.success && mcpResult.data) {
        ...
        case 'get_validator_status': {
            const validatorKey = toolInput.validator_key;
            const mcpResult = await (0, mcp_1.callMcpTool)('get_validators', {});
            if (mcpResult.success && mcpResult.data) {
                return JSON.stringify(mcpResult.data);
            }
            const match = context.delegationInfo.validators.find((v) => v.validatorKey === validatorKey);
            return JSON.stringify({
...
async function agentLoop(apiKey, messages, context) {
    ...
    for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
        ...
        for (const toolCall of toolCalls) {
            ...
            const toolResult = await executeMcpTool(toolCall.function.name, args, context);
            messages.push({
                role: 'tool',
...
function deterministicDecision(context) {
    ...
    const warnings = riskResult.factors.map((f) => f.description);
    let urgency = 'low';
    if (riskResult.level === 'warning')
        urgency = 'medium';
 ...
 */
async function runAgentDecision(context) {
    ...
    try {
        ...
        const validActions = ['rebalance', 'alert', 'hold'];
...
${recentDeploys.length > 0
        ? recentDeploys.slice(0, 5).map((d) => `- ${d.timestamp} | ${d.status} | cost: ${d.cost} CSPR | hash: ${d.deployHash.slice(0, 16)}...`).join('\n')
 ...
 */
async function getAgentDecision(riskResult, walletData, delegationInfo) {
