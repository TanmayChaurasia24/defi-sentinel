...
function getCsprCloudToken(): string | null {
...
let _cachedKeyPair: Keys.AsymmetricKey | null = null;

function loadKeyPair(): Keys.AsymmetricKey | null {
  ...
  if (keyPathRaw.endsWith('.pem')) {
    ...
    try {
      _cachedKeyPair = Keys.Ed25519.loadKeyPairFromPrivateFile(keyPath);
      console.log(`[contract] 🔑 Loaded Ed25519 key from ${keyPath}`);
      return _cachedKeyPair;
    } catch {
      try {
        _cachedKeyPair = Keys.Secp256K1.loadKeyPairFromPrivateFile(keyPath);
  ...
  try {
    ...
    _cachedKeyPair = Keys.Ed25519.parseKeyPair(
 ...
 */
async function broadcastDeploy(signedDeploy: DeployUtil.Deploy): Promise<string> {
...
export async function writeRiskScore(
  ...
  if (!isContractConfigured()) {
    console.log(
      `[contract] 📝 STUB update_risk_score(wallet=${walletAddress.slice(0, 12)}..., score=${score})`
    );
  ...
  try {
    ...
    const walletKey = CLPublicKey.fromHex(walletAddress);

    const args = RuntimeArgs.fromMap({
      wallet: walletKey,
      score: CLValueBuilder.u8(Math.min(score, 100)),
    });

    const deploy = DeployUtil.makeDeploy(
      new DeployUtil.DeployParams(keyPair.publicKey, CHAIN_NAME, 1, 1800000),
      DeployUtil.ExecutableDeployItem.newStoredContractByHash(
        contractHashBytes,
        'update_risk_score',
        args
      ),
      DeployUtil.standardPayment(GAS_PAYMENT)
    );
 ...
 */
export async function writeRebalance(
  walletAddress: string,
  deployHash: string,
  score: number
): Promise<ContractWriteResult> {
  if (!isContractConfigured()) {
    console.log(
      `[contract] 📝 STUB log_rebalance(wallet=${walletAddress.slice(0, 12)}..., hash=${deployHash}, score=${score})`
    );
  ...
  try {
    ...
    const args = RuntimeArgs.fromMap({
      wallet: walletKey,
      deploy_hash: CLValueBuilder.string(deployHash),
      score: CLValueBuilder.u8(Math.min(score, 100)),
    });
...
export async function writeAction(
  ...
  if (!isContractConfigured()) {
    console.log(
      `[contract] 📝 STUB log_action(wallet=${walletAddress.slice(0, 12)}..., action=${actionType}, score=${result.score})`
    );
    return { success: true, dryRun: true };
  }
  ...
  try {
    ...
    const args = RuntimeArgs.fromMap({
      wallet: walletKey,
      action_type: CLValueBuilder.string(actionType),
      score: CLValueBuilder.u8(Math.min(result.score, 100)),
    });
...
// ─────────────────────────────────────────────────────────────────────────────
// Additional helpers (kept for backward compatibility)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function logAlertOnChain(
  ...
  if (!isContractConfigured()) {
    console.log(
      `[contract] 📝 STUB log_alert(wallet=${walletAddress.slice(0, 12)}..., score=${riskScore}, warnings=${warnings.length})`
    );
    return;
  }
  ...
  try {
    ...
    const args = RuntimeArgs.fromMap({
      wallet: walletKey,
      action_type: CLValueBuilder.string('alert'),
      score: CLValueBuilder.u8(Math.min(riskScore, 100)),
    });
    ...
    console.log(
      `[contract] 🔔 log_alert → ${deployHash} (score=${riskScore})`
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
 ...
 */
export async function getContractActionHistory(
